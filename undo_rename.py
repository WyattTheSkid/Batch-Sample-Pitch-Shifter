...
for filename in os.listdir(current_dir):
    ...
    if match:
        midi_number = int(match.group(1)) 
        extension = match.group(2)  
      
        if midi_number in midi_numbers:
            note_name = midi_numbers[midi_number]
            new_filename = f"{note_name}.{extension}"
            old_filepath = os.path.join(current_dir, filename)
            new_filepath = os.path.join(current_dir, new_filename)
            os.rename(old_filepath, new_filepath)
            print(f"Renamed {filename} back to {new_filename}")

print("Undo renaming complete.")
